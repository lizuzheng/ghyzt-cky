...
public class DoubleDatePickerDialog extends AlertDialog implements OnClickListener, OnDateChangedListener {
	...
	public DoubleDatePickerDialog(Context context, int theme, OnDateSetListener callBack, int year, int monthOfYear,
		...
		//设置最大时间值
		mDatePicker_start.setMaxDate(new Date().getTime());
		
      String min="2014-04-01"; 
      SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");  
      Date d2=null;
      try {d2 = sdf.parse(min);} catch (ParseException e) {e.printStackTrace();}

		mDatePicker_start.setMinDate(d2.getTime());
		
		mDatePicker_end = (DatePicker) view.findViewById(R.id.datePickerEnd);
		
		mDatePicker_end.setMaxDate(new Date().getTime());
		mDatePicker_end.setMinDate(d2.getTime());
		
		mDatePicker_start.init(2016, 3, 1, this);
